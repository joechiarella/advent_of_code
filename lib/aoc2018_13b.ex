defmodule AOC2018_13B do
  defmodule Cart do
    defstruct pos: {0, 0}, dir: :s, turns: :left
  end
  ...
  def tick(carts, tracks) do
    ...
    # IO.puts "Number of carts: #{length(carts)}"
    if length(carts) > 1 do
      tick(carts, tracks)
    else
      hd(carts).pos
    end
  ...
  def move_carts([next | rest], moved, tracks) do
    ...
    other = check_collision(now.pos, rest ++ moved)

    if other == :none do
      move_carts(rest, [now | moved], tracks)
    else
      rest = List.delete(rest, other)
      moved = List.delete(moved, other)
      move_carts(rest, moved, tracks)
    end
  ...
  def check_collision(pos, others) do
    Enum.find(others, :none, fn other -> other.pos == pos end)
  end
